...
% ADD MORE DESCRIPTION OF EACH BLOCK HERE !!!!!
...
\node[box, below=of embed] (features) {Features\\$(B,W,C,N)$};
...
\node[box, below=of attn] (context) {Context\\$(B,C,N)$};
...
\node[output, below=of corr] (output) {$f_{\text{next}}$};
...
\draw[arrow, dashed, blue] ([yshift=0.4cm]features.north east) 
    to[out=30,in=150] node[above, font=\footnotesize, black]{residual} 
    ([yshift=0.4cm]context.north west);

% FIXED: LEFT→DOWN→RIGHT path from input to output
\draw[arrow, dashed, orange, thick] ([xshift=-0.8cm]input.south west) 
    |- ++(0,-0.8) 
    -| ([xshift=-0.8cm]output.south west)
    node[below, font=\footnotesize, pos=0.7]{$u_t$};
...
% ------------- NFTM ARCHITECTURE WITH TCAN CONTROLLER FIGURE -------------
\begin{figure}[h!]
\centering
\begin{tikzpicture}[node distance=1.5cm and 1cm, auto,
    block/.style={rectangle, draw, thick, text width=2.5cm, 
                  text centered, minimum height=1cm, font=\small},
    arrow/.style={thick,->,>=stealth}]

% Input History
\node[block] (input) {History $u_{t-W:t}$ \\ $(B,W,P)$};

% Attention Block
\node[block, right=of input] (attn) {CausalTemporalAttention \\ Embed → QKV → Causal Attn \\ + GroupNorm};

% Decoder
\node[block, right=of attn] (decoder) {Decoder \\ Conv1D(32) → BN → GELU \\ Conv1D(1)};

% Output
\node[block, right=of decoder] (output) {$u_{t+1} = u_t + \tanh(\delta u)$};

% Window update (dashed)
\node[block, below=0.5cm of attn] (window) {Shift Window: \\ $[u_{t-W+1:t}, u_{t+1}]$};

% Arrows
\draw[arrow] (input) -- (attn);
\draw[arrow] (attn) -- (decoder);
\draw[arrow] (decoder) -- (output);
\draw[arrow, dashed] (output) |- (window);
\draw[arrow, dashed] (window) -| ([yshift=-0.5cm]input.south east);

% Loss feedback (curved)
\draw[arrow, red, thick] ([xshift=0.5cm]output.north east) 
    to[bend right=30] node[above, font=\tiny] {MSE + Physics} 
    ([yshift=0.3cm]input.north west);

\end{tikzpicture}
\caption{NFTM Architecture: Causal autoregressive prediction with physics-informed training.}
\label{fig:nftm_arch}
\end{figure}
\noindent Figure~\ref{fig:nftm_arch} summarizes the overall architecture of the NFTM model using the TCAN as controller. The model processes a sliding window of past fields to predict the next field, employing causal attention and convolutional decoding. The autoregressive rollout is indicated by dashed arrows, while the red feedback loop represents the physics-informed loss used during training.
...
\toprule
\textbf{Metric} & & \textbf{Purpose} & \textbf{Good} & \textbf{Failure} \\
\midrule[1pt]
...
\hline[0.8pt]
Mass Err. & $|\int u\,dx-M_0|$ & Conservation & $<0.01$ & $>0.1$ \\
Energy Mono. & $E(t+1)\leq E(t)$ & Dissipation & $>0.95$ & $<0.8$ \\
PDE Res. & PDE residual & PDE satisfaction & $<0.01$ & $>0.1$ \\
Max Grad. & $\max|\partial_xu|$ & Shock preserv. & $\pm10\%$GT & Diverges \\
Grad. Err. & Grad. difference & Shock sharpness & $<0.05$ & $>0.2$ \\
\bottomrule[1pt]
\end{tabular}
}
\caption{Evaluation Metrics for NFTM Performance Assessment}
\label{tab:nftm_metrics}
\end{table}

Table~\ref{tab:nftm_metrics} summarizes the evaluation metrics used to assess NFTM performance, detailing formulas, purposes, and thresholds for good vs. failure cases. This suite ensures comprehensive assessment of both numerical accuracy and physical fidelity.
