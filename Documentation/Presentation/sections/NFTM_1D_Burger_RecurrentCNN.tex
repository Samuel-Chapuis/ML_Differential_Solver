...
% \node[layer] (field) {$f_t \in \mathbb{R}^{N}$};
% \node[layer, right=of field] (nu) {$\nu$};
...
% \node[layer, right=of delta] (sum) {$f_{t+1} = f_t + \Delta f_t$};

% \draw[->] (field) -- (cat);
% \draw[->] (nu.south) |- (cat.east);
